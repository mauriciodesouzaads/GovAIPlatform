...
import { BookOpen, Package, Terminal, Copy, CheckCircle2, ExternalLink, Clock, AlertTriangle } from 'lucide-react';
  ...
  -H "x-org-id: $GOVAI_ORG_ID"`;
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto space-y-5">
            ...
            {/* 6c.B.3 CP1.D-B: SDK warning — @govai/sdk e govai-sdk não
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
                            Os pacotes <code className="font-mono text-xs px-1.5 py-0.5 rounded bg-secondary text-foreground">@govai/sdk</code> (npm)
                            e <code className="font-mono text-xs px-1.5 py-0.5 rounded bg-secondary text-foreground ml-1">govai-sdk</code> (PyPI)
                            ainda não estão publicados. Use a{' '}
                            <strong className="text-foreground">API REST diretamente</strong> via cURL
                            ou gere clients localmente com o spec OpenAPI{' '}
                            <Link href="/v1/docs" target="_blank" className="text-primary hover:underline inline-flex items-center gap-1">
                                openapi.yaml<ExternalLink className="w-3 h-3 inline" />
                            </Link>
                            {' '}e <code className="font-mono text-xs px-1.5 py-0.5 rounded bg-secondary text-foreground">openapi-generator-cli</code>.
                        </p>
                        <p className="text-xs text-muted-foreground/70 mt-2">
                            Os exemplos TypeScript e Python abaixo são prévia da experiência final.
                        </p>
                    </div>
                </div>
            </div>
