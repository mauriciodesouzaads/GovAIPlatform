    ...
    const worker = new Worker<ArchitectJobPayload>(
        ...
        async (job: Job<ArchitectJobPayload>) => {
            ...
            if (job.name === 'resolve-approval') {
                const { orgId, workItemId, promptId, approved, actorEmail } = job.data as ResolveApprovalPayload;
                console.log(`[Architect Worker] resolve-approval for ${workItemId} promptId=${promptId} approved=${approved}`);
